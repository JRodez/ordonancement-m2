...
#include "Th_Robot_Ret.h"
#include "RTDS_messages.h"
#include "RTDS_gen.h"

#define RTDS_PROCESS_NUMBER RTDS_process_Th_Robot_Ret
#define RTDS_PROCESS_NAME Th_Robot_Ret

/*
** PROCESS Th_Robot_Ret:
** ---------------------
*/

RTDS_TASK_ENTRY_POINT(Th_Robot_Ret)
  {
  ...
  #include "Th_Robot_Ret_tempVars.h"

  RTDS_myLocals[0] = (void*)&idP;
  RTDS_myLocals[1] = (void*)&panne;
    ...
    if ( RTDS_currentContext->currentMessage != NULL )
      ...
      switch(RTDS_currentContext->sdlState)
        ...
        case RTDS_state_attConv:
          switch(RTDS_currentContext->currentMessage->messageNumber)
            ...
            case RTDS_message_Libre:
              RTDS_RESET_TIMER(RTDS_message_defaillRobotRet);
              RTDS_MSG_QUEUE_SEND_TO_ID(RTDS_message_retraitConf, 0, NULL,               idP);
              RTDS_SDL_STATE_SET(RTDS_state_Idle);
              break;
            /* Transition for state attConv - message retraitReq */
            case RTDS_message_retraitReq:
              RTDS_MSG_SAVE(RTDS_currentContext->currentMessage);
              break;
            /* Transition for state attConv - message defaillRobotRet */
            case RTDS_message_defaillRobotRet:
              RTDS_SDL_STATE_SET(RTDS_state_DEfaillanceRobotRetrait);
              break;
            default:
        ...
        case RTDS_state_Idle:
          switch(RTDS_currentContext->currentMessage->messageNumber)
            {
            /* Transition for state Idle - message retraitReq */
            case RTDS_message_retraitReq:
              RTDS_SET_TIMER(RTDS_message_defaillRobotRet, 1000 * (0 + 20));
              idP = SENDER;

              RTDS_MSG_QUEUE_SEND_TO_NAME(RTDS_message_retP, 0, NULL, "gestionConvoyeur", RTDS_process_gestionConvoyeur);
